...
if (isset($_POST['dangkyad'])) {
  $tenkhachhang = $_POST['ten_ad'];
  $username = $_POST['username'];
  $password = md5($_POST['password']);
  $diachi = $_POST['diachi'];
  $dienthoai = $_POST['dienthoai'];
  $sql_dangky = mysqli_query($mysqli, ("INSERT INTO admin(ten_admin,username,password,diachi_admin,dienthoai_admin) VALUE(
            '" . $tenkhachhang . "','" . $username . "','" . $password . "','" . $diachi . "','" . $dienthoai . "')"));
  if ($sql_dangky) {
    echo '<p style="color:green">Bạn đã đăng ký thành công</p>';
    $_SESSION['dangkyad'] = $tenkhachhang;
...
<body>
  <div class="main-bg">
    <div class="login-box">
      <h1>ĐĂNG KÝ CHO QUẢN LÝ</h1>
      <div class="login-form">
        <!-- <h2>login Now</h2> -->
        <form method="POST">
          <div class="input-box">
            <input type="text" placeholder="Họ Và Tên" required name="ten_ad" />
          ...
          <button type="submit" class="login-btn" name="dangkyad">REGISTER</button>
